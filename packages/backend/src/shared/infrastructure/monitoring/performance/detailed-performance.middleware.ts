...
export const detailedPerformanceMiddleware = new Elysia()
  ...
  .onBeforeHandle(({ request, store }) => {
    ...
    redis
      .incr(PERFORMANCE_CONFIG.REDIS_KEYS.API_ACTIVE_CONNECTIONS)
      .catch((err) => {
  ...
  .onAfterHandle(({ store, set }) => {
    ...
    redis
      ...
      .then(() =>
        redis.ltrim(PERFORMANCE_CONFIG.REDIS_KEYS.API_RESPONSE_TIMES, 0, 999)
      )
      .catch((err) => console.warn("Failed to store response time:", err));

    // Store per-endpoint metrics
    redis
      .lpush(`${endpointKey}:response_times`, responseTime.toString())
      .then(() => redis.ltrim(`${endpointKey}:response_times`, 0, 100))
      .catch((err) =>
        console.warn("Failed to store endpoint response time:", err)
      );

    redis
      .incr(PERFORMANCE_CONFIG.REDIS_KEYS.API_TOTAL_REQUESTS)
      .catch((err) => {
  ...
  .onError(({ store, error }) => {
    ...
    redis
      .incr(PERFORMANCE_CONFIG.REDIS_KEYS.API_ERROR_REQUESTS)
      .catch((err) => {
