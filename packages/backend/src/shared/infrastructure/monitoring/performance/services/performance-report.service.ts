...
export class PerformanceReportService {
  ...
  private static calculateAverages(metrics: PerformanceMetrics[]) {
    return {
      api: {
        responseTime: MetricsCalculator.average(
          metrics.map((m) => m.api.responseTime)
        ),
        throughput: MetricsCalculator.average(
          metrics.map((m) => m.api.throughput)
        ),
        errorRate: MetricsCalculator.average(
          metrics.map((m) => m.api.errorRate)
        ),
      },
      database: {
        queryTime: MetricsCalculator.average(
          metrics.map((m) => m.database.queryTime)
        ),
        connectionPoolUsage: MetricsCalculator.average(
          metrics.map((m) => m.database.connectionPoolUsage)
        ),
        slowQueries: MetricsCalculator.sum(
          metrics.map((m) => m.database.slowQueries)
        ),
      },
      redis: {
        hitRate: MetricsCalculator.average(metrics.map((m) => m.redis.hitRate)),
        memoryUsage: MetricsCalculator.average(
          metrics.map((m) => m.redis.memoryUsage)
        ),
